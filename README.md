...
In this assignment, you will develop a ROS 2 package to autonomously navigate a TurtleBot4 through a custom simulated Gazebo environment (world:=level2). The robot will spawn at the initial starting coordinates of X: -5.0, Y: 0.0, Yaw: 3.14159.
...
The final waypoint (4.9, 2.0, 1.5708) correctly aligns the robot to face a designated area containing three differently colored spheres. Once the robot stops at this location, it must:
...
## Bonus: SLAM Integration
Students who successfully implement Simultaneous Localization and Mapping (SLAM) during the navigation phase will receive full bonus points. Rather than relying purely on blind odometry, you should integrate slam_toolbox or a similar SLAM algorithm to map the level2 environment dynamically as the robot navigates the waypoints.
